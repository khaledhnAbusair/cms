...
public class PageManagementController {
	...
	public String editPage(@ModelAttribute DraftPageEntity draftPageEntity, @RequestParam(name = SITE) Integer siteId,
		...
		pageUseCase.editPage(draftPageEntity, siteId, publishedPageUrl);
	...
	@RequestMapping(path = EDIT_PAGE, params = "publish", method = RequestMethod.POST)
	public String publishPage(@RequestParam(name = SITE) Integer siteId, @RequestParam(name = PAGE_URL) String pageUrl,
			UriComponentsBuilder builder) {
		pageUseCase.publishPage(pageUrl, siteId);
		return REDIRECT + builder.path(PAGE_MANAGEMNT).build().toUriString();
	}
