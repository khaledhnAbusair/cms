...
public class DeliveryControllers {
	...
	public String site(@PathVariable(name = SITE_URL) String siteUrl, Model model) {
		model.addAttribute(PAGE_CONTENT, pageUseCase.loadLandingPage(siteUrl).getPageContent());
		model.addAttribute(PAGES, siteUseCase.getAllPagesBySiteUrl(siteUrl));
		return BASE_JSP_PAGE;
	}
